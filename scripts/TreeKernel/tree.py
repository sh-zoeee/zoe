...
"""
    このファイルはPython2.xxが前提で書かれた
    https://github.com/shreyas725/Tree-Kernel
    のコードをPython3.11.7で扱うために削除されている関数などを修正したもの
"""
...
class TreeNode:
    ...
    def __init__(self,val=None, chs=list()):
    ...
    def __str__(self):
        ...
        while stack:
            ...
            if visited:
                if node.chs:
                    children_str = ",".join(result.pop() for _ in range(len(node.chs)))
                    result.append(f"{node.val}({children_str})")
                else:
                    result.append(node.val)
            else:
                stack.append((node, True))
                stack.extend((child, False) for child in node.chs)
    ...
    def addkid(self, node, before=False):
        if before:  self.chs.insert(0, node)
        else:   self.chs.append(node)
        return self
