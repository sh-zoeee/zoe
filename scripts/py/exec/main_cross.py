...
TREEBANK_DIR = "data/original/treebank/"
...
def main():
    ...
    corpora = list()
    corpora.append(f"{HOME_DIR}{TREEBANK_DIR}corpora/English/English-ESL.conllu")
    corpora.append(f"{HOME_DIR}{TREEBANK_DIR}corpora/English/English-Atis.conllu")
    #corpora.append(f"parsed/En-EWT-spacy.conllu")
    corpora.sort()
    CORPORA = corpora
    
    
    src = CORPORA[0].split("/")[-1].split(".")[0]
    print(src, end=" vs ", flush=True)
    dir_name = src + "_"
    tar = CORPORA[1].split("/")[-1].split(".")[0]
    print(tar, end="\n", flush=True)
    dir_name += tar
